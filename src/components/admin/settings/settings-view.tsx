          ...
          <CardHeader className="p-4 border-b border-slate-100">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <CardDescription className="text-xs text-slate-500">
                  Configure live payment credentials, webhook secrets, and checkout settings securely.
                </CardDescription>
              </div>

              <Badge variant="outline" className={`font-mono text-xs ${
                systemSettings?.razorpay?.isLive ? "bg-emerald-50 text-emerald-700 border-emerald-300" : "bg-amber-50 text-amber-700 border-amber-300"
              }`}>
                {systemSettings?.razorpay?.isLive ? "LIVE CONFIGURED" : "CREDENTIALS REQUIRED"}
              </Badge>
            </div>
          </CardHeader>
          <CardContent className="p-4 space-y-4 max-w-2xl">
            {/* Honest instructions banner */}
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-800 space-y-1">
              <p className="font-bold flex items-center gap-1.5">
                <KeyRound className="h-3.5 w-3.5" /> Razorpay Integration Instructions:
              </p>
              <p className="text-[11px] leading-relaxed">
                1. Log into your Razorpay Dashboard at <strong>dashboard.razorpay.com</strong>.<br />
                2. Navigate to <strong>Settings &rarr; API Keys</strong> and generate your <strong>Key ID</strong> and <strong>Key Secret</strong>.<br />
                3. Go to <strong>Settings &rarr; Webhooks</strong> and add webhook URL: <code className="bg-white px-1 py-0.5 rounded font-mono">https://www.softlabglobal.com/api/webhooks/razorpay</code><br />
                4. Enter the secrets below. Secrets are stored securely server-side and never exposed to client browsers.
              </p>
            </div>
            ...
            >
              <Save className="h-4 w-4 mr-1" />
              {updateSettingsMutation.isPending ? "Saving..." : "Save Razorpay Credentials"}
            </Button>
          </CardContent>
        </Card>
      )}
